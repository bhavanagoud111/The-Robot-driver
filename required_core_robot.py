"""
Required Core: The Robot Driver
A Python program that controls a web browser using Playwright to complete a fixed, single task.
This implementation demonstrates core Python software engineering and Playwright skills.
"""
...
class ProductSearchTask:
    ...
    async def execute_product_search(self) -> TaskResult:
        """
        Execute the complete product search task
        
        Task Flow:
        1. Navigate to an e-commerce site
        ...
        try:
            ...
            async with self.robot as robot:
                ...
                if not nav_result.success:
                    return TaskResult(
                        success=False,
                        message="Failed to navigate to website",
                ...
                # Look for search functionality (this site has a search form)
                ...
                # Type search query
                type_result = await robot.type_text(search_input, "python programming")
                ...
                # Click search button
                click_result = await robot.click_element(search_button)
                if not click_result.success:
                    return TaskResult(
                        success=False,
                        message="Failed to click search button",
                ...
                product_selector = '.product_pod'
                product_wait = await robot.wait_for_element(product_selector, timeout=15000)
                
                if not product_wait.success:
                    return TaskResult(
                        success=False,
                ...
                product_title_selector = '.product_pod h3 a'
                product_price_selector = '.product_pod .price_color'
                
                # Get product title
                title_result = await robot.get_text_content(product_title_selector)
                ...
                # Get product price
                price_result = await robot.get_text_content(product_price_selector)
                ...
                logger.info(f"✅ Product found: {product_title}")
